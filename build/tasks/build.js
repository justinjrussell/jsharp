/**
 * Special concat/build task to handle various jSharp build requirements
...
module.exports = function (grunt) {
	...
	function convert(name, path, contents) {
		...
		if (/.\/var\//.test(path)) {
			...
			// Ignore jSharp's exports (the only necessary one)
			if (name !== "jsharp") {
				contents = contents
		...
		if ((amdName = grunt.option("amd")) != null && /^exports\/amd$/.test(name)) {
			if (amdName) {
				grunt.log.writeln("Naming jSharp with AMD name: " + amdName);
			} else {
				grunt.log.writeln("AMD name now anonymous");
			}
			// Remove the comma for anonymous defines
			contents = contents
				.replace(/(\s*)"jsharp"(\,\s*)/, amdName ? "$1\"" + amdName + "\"$2" : "");
